...
export default async function WatchPage({ params }: Props) {
  ...
  const related = primaryGenre ? await getRelatedTitles(asset_id, primaryGenre) : []
  const initialSecs = progress?.progress_secs ?? 0
  const resumeLabel = initialSecs > 60
    ? `Resuming from ${Math.floor(initialSecs / 60)}m ${Math.floor(initialSecs % 60)}s`
    : null
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-6 pb-6 border-b border-white/[0.06]">
              ...
              <div className="mt-4 flex gap-4 text-xs text-white/30">
                ...
                {title.duration_mins && (
                  <span>{Math.floor(title.duration_mins / 60)}h {title.duration_mins % 60}m</span>
                )}
