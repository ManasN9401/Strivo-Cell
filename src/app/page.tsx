...
import {
  getTitlesByGenre, getNewTitles,
  getWatchlistIds, getContinueWatching,
    ...
    <section className="py-4 max-w-content mx-auto px-8">
      ...
      <div className="flex gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="flex-shrink-0 w-[200px] aspect-[2/3] bg-cinema-surface rounded-lg animate-pulse"
...
async function HomeRows() {
  const [action, scifi, drama, thriller, horror, newTitles, watchlistIds, continueItems] =
    await Promise.all([
  ...
  return (
    <>
      ...
      {rows.map(({ heading, titles, genre }) =>
        titles.length ? (
          <ContentRow key={heading} heading={heading} titles={titles}
