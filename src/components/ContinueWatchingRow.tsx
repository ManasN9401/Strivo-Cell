  ...
  return (
    <section className="max-w-content mx-auto px-8 pt-10 pb-2">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4
        ...
        {items.map(({ title, progress_secs, duration_mins, episode }) => {
          const pct = duration_mins
            ? Math.min((progress_secs / (duration_mins * 60)) * 100, 100) : 0

          const href = episode ? `/watch/episode/${episode.id}` : `/watch/${title.id}`

          return (
            <Link
              key={episode ? episode.id : title.id}
              href={href}
            ...
            >
              <div className="relative aspect-video rounded-lg overflow-hidden bg-strivo-surface
                              transition-transform duration-200 group-hover:scale-105 shadow-lg">
                {title.backdrop_path ? (
                  <Image src={title.backdrop_path} alt={title.title} fill
                ...
                <div className="absolute bottom-0 inset-x-0 h-0.5 bg-white/20">
                  <div className="h-full bg-gradient-to-r from-blue-500 via-cyan-400 to-cyan-200 shadow-[0_0_8px_rgba(34,211,238,0.5)]" 
                       style={{ width: `${pct.toFixed(1)}%` }} />
                </div>
              </div>

              <div className="mt-2 px-1">
                <p className="text-white text-sm font-semibold truncate group-hover:text-strivo-accent transition-colors">
                  {title.title}
                </p>
                {episode ? (
