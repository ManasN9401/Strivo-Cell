    ...
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label="Save to playlist"
           className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                      w-full max-w-sm bg-strivo-surface border border-white/10
            ...
            return (
              <li key={pl.id}>
                <button onClick={() => togglePlaylist(pl.id)} disabled={isPending}
                  ...
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-all ${isSaved ? 'bg-strivo-accent border-strivo-accent' : 'border-white/20'}`} aria-hidden>
                    {isSaved && (
          ...
          {creating ? (
            <div className="flex gap-2">
              <input type="text" value={newName} onChange={e => setNewName(e.target.value)}
                     ...
                     className="flex-1 bg-strivo-bg border border-white/10 rounded-lg px-3 py-2
            ...
            <button onClick={() => setCreating(true)}
                    className="flex items-center gap-2 text-strivo-accent text-sm font-medium
